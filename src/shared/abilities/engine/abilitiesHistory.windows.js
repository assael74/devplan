// C:\projects\devplan\src\shared\abilities\engine\abilitiesHistory.windows.js

// קובץ מקביל: functions/src/domain/abilities/engine/abilitiesHistory.windows.js
...
import { normalizeAbilities, normalizeStoredForm } from './abilitiesHistory.forms.js'

const DAY_MS = 24 * 60 * 60 * 1000
const MAX_WINDOW_DAYS = 56 // 8 weeks
...
export function groupFormsByWindow(forms = []) {
...
export function mergeWindowAbilitiesHistory(windows = []) {
  if (!windows.length) {
    return {
  ...
  for (let i = 1; i < windows.length; i += 1) {
    ...
    for (const item of abilitiesList) {
      ...
      if (newVal == null && oldVal == null) next[id] = null
        ...
        next[id] = roundToHalf(
          oldVal * strategy.oldWeight + newVal * strategy.newWeight
        )
      }
    }
