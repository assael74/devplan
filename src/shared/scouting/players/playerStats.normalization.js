const cleanValue = value => String(value === null || value === undefined ? '' : value).trim()
const hasValue = value => value !== undefined && value !== null && cleanValue(value) !== ''
...
export const normalizePlayerStats = (player = {}) => {
  ...
  return {
    ...
    substituteIn: toNumberOrZero(resolveStatValue({ stats, source: player, key: 'substituteIn', legacyKeys: ['subIn'] })),
    substitutedOut: toNumberOrZero(resolveStatValue({ stats, source: player, key: 'substitutedOut', legacyKeys: ['subOut'] })),
    ...
    teamRank: resolveOptionalContextValue({ stats, source: player, key: 'teamRank' }),
    teamGoalsFor: toNumberOrZero(resolveStatValue({ stats, source: player, key: 'teamGoalsFor' })),
    teamGoalsAgainst: toNumberOrZero(resolveStatValue({ stats, source: player, key: 'teamGoalsAgainst' })),
    teamAttackPerformance: resolveOptionalContextValue({ stats, source: player, key: 'teamAttackPerformance' }),
    teamDefensePerformance: resolveOptionalContextValue({ stats, source: player, key: 'teamDefensePerformance' }),
    minutesPerGame: toNumberOrZero(hasValue(minutesPerGame) ? minutesPerGame : (games ? minutes / games : 0)),
    goalsPer90: toNumberOrZero(hasValue(goalsPer90) ? goalsPer90 : (minutes ? (goals * 90) / minutes : 0)),
  }
}
