import { buildPlayerScoutResult as buildSharedPackageResult } from '@devplan/players-scout-engine/players/index.js'
import {
...
test('shared package preserves the existing Scout engine result exactly', () => {
  const input = {
    player: { primaryPosition: 'defense', games: 20, goals: 4, minutes: 1407 },
    team: { ageGroupId: 'u14', clubId: 'club_a', clubLevel: 2, clubStrengthLevel: 2, leagueLevel: 3, gamesPlayed: 23, leagueGameTime: 80, defense: { priorityLevel: 'negative' } },
    season: { seasonKey: '25/26', seasonStatus: 'completed', leagueNumGames: 23 },
    perspective: 'players_database',
  }

  expect(buildSharedPackageResult(input)).toEqual(buildPlayerScoutResult(input))
})
