// src/shared/scouting/teams/teamScout.builder.js

import { addTeamScoutRankings } from './teamScout.rankings.js'
import { buildTeamScoutContext } from './context/teamContext.js'
import { buildTeamNeeds } from './opportunity/teamNeeds.js'
import { buildTeamRecruitmentOpportunity } from './opportunity/teamRecruitmentOpportunity.js'
import {
...
const normalizeTeamScoutRow = (row = {}, index = 0) => {
  ...
  return {
    ...
    clubLevel: pickFirstNumber(
      row.clubLevel,
      row.club && row.club.level,
      row.team && row.team.clubLevel
    ),
    clubStrengthLevel: pickFirstNumber(
      row.clubStrengthLevel,
      row.club && row.club.clubStrengthLevel,
      row.team && row.team.clubStrengthLevel,
      row.clubLevel,
      row.club && row.club.level,
      row.team && row.team.clubLevel
    ),
...
export const buildTeamScoutLeagueModel = ({
  ...
  const resultRows = rankedRows.map((row) => {
    const performanceRow = buildTeamScoutPerformance({
      row,
      leagueLevel,
      leagueNumGames,
    ...
    const futureCompetitionPath =
      futureCompetitionByTeamId[performanceRow.teamId] ||
      performanceRow.source?.futureCompetitionPath ||
      null
    const context = buildTeamScoutContext({
      row: performanceRow,
      clubLevel: performanceRow.clubLevel || clubLevel,
      clubStrengthLevel: performanceRow.clubStrengthLevel || clubStrengthLevel || performanceRow.clubLevel || clubLevel,
      leagueLevel,
      futureCompetitionPath,
    })
    const needs = buildTeamNeeds({
      offense: performanceRow.offense,
      defense: performanceRow.defense,
    })
    const recruitmentOpportunity = buildTeamRecruitmentOpportunity({
      needs,
      context,
    })
