// src/ui/forms/gameStats/logic/entry.logic.js

const isRateParm = param => {
  return param?.id?.toLowerCase().includes('rate')
}

const isTotalParm = param => {
  return param?.id?.toLowerCase().endsWith('total')
}
...
const isSuccessParm = param => {
  return !isTotalParm(param) && !isRateParm(param)
}
...
const buildRegularField = param => {
  return {
    id: param.id,
    type: param.statsParmFieldType || 'number',
    statsParmType: param.statsParmType || 'general',
    param,
  }
}
...
export const buildEntryFields = params => {
  const regular = []
  const triplets = {}

  ;(params || []).forEach(param => {
    if (param.statsParmFieldType !== 'triplet') {
      regular.push(buildRegularField(param))
      return
    }

    const group = param.tripletGroup || param.id
    triplets[group] = [...(triplets[group] || []), param]
  })
