...
import { getStartOfWeekSunday } from './schedule.date.js'
import { addDaysToWeekId, buildWeekMeta } from './schedule.week.js'
import { buildWeekRangeLabel } from './schedule.format.js'
...
const buildWeekSummary = ({ allRows = [], currentWeekRows = [], nextWeekRows = [] } = {}) => {
  return {
...
export const buildScheduleModel = ({
  ...
  return {
    ...
    currentWeekRangeLabel: buildWeekRangeLabel(
      currentMeta.startDate,
      currentMeta.endDate
    ),

    nextWeekRangeLabel: buildWeekRangeLabel(
      nextMeta.startDate,
      nextMeta.endDate
    ),
  }
