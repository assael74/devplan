// src/ui/fields/core/YearPicker.js
...
export default function YearPicker({
  ...
  const list = React.useMemo(() => {
    ...
    if (!arr) {
      const past = Math.max(0, Number(range && range.past !== null && range.past !== undefined ? range.past : 15))
      const future = Math.max(0, Number(range && range.future !== null && range.future !== undefined ? range.future : 0))
