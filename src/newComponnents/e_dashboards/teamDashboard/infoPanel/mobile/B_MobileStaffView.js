...
export default function MobileStaffView({
  ...
  const coach = formData.proMan?.find((staff) => staff.role === 'coach') || {};
  const otherStaff = formData.proMan?.filter((staff) => staff.role !== 'coach') || [];
              ...
              <Grid xs={12}>
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    handleStaffChange(formData.proMan.findIndex(p => p.role === 'coach'), 'fullName')({ target: { value: val } });
                  }}
                  size="sm"
                  fullWidth
                />
              ...
              <Grid xs={12}>
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    handleStaffChange(formData.proMan.findIndex(p => p.role === 'coach'), 'phone')({ target: { value: val } });
                  }}
                  size="sm"
                  fullWidth
                />
                        ...
                        <Grid container spacing={1}>
                          <Grid xs={6}>
                            <Input
                              ...
                              onChange={(e) => {
                                const val = e.target.value.replace(/[^A-Za-zא-ת\s]/g, '').slice(0, 20);
                                handleStaffChange(formData.proMan.findIndex(p => p.role === staff.role), 'fullName')({ target: { value: val } });
                              }}
                              size="sm"
                              fullWidth
                            />
                          ...
                          <Grid xs={6}>
                            <Input
                              ...
                              onChange={(e) => {
                                const val = e.target.value.replace(/\D/g, '').slice(0, 10);
                                handleStaffChange(formData.proMan.findIndex(p => p.role === staff.role), 'phone')({ target: { value: val } });
                              }}
                              size="sm"
                              fullWidth
                            />
