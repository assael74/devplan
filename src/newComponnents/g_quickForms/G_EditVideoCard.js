...
import VideoEditModalContent from './modalContent/G_VideoEditModalContent.js'
...
function DialogTitleModal({ item, actionItem, isMobile }) {
  const actionLabel = {
    videoInfo: 'עריכת שם הוידאו',
    videoComments: 'עריכת הערות לוידאו',
    videoTags: 'עריכת תגי הוידאו',
    videoAnalysisInfo: 'עריכת שם הוידאו',
    videoAnalysisLinks: 'עריכת קישור הוידאו',
    delete: 'מחיקת וידאו',
  }[actionItem] || '';
...
export default function EditVideoCard({
  item,
  actions,
  columns,
  onSubmit,
  ...
  const {
    ...
    onSubmit: async (data) => {
      ...
      try {
        await onSubmit?.(data);
        showSnackbar('וידאו עודכן בהצלחה', 'warning', 'videos');
        setActionItem('');
      } catch (err) {
        console.error('❌ שגיאה בעדכון:', err);
        showSnackbar('שגיאה בעדכון הוידאו', 'warning', 'videos');
      } finally {
        setIsUpdate(false);
      }
    },
