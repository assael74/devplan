// src/features/hub/teamProfile/sharedUi/players/print/PerformancePrintContent.js
...
function MetricChip({ item }) {
  const tone = METRIC_TONES[item.metricKey] || METRIC_TONES.neutral
...
function PerformanceTopItems({ items = [] }) {
  return (
...
function PerformanceRow({ row }) {
  return (
    <Box component='tr' className='dpPrintRow'>
      ...
      <Box component='td' sx={[sharedSx.td, sharedSx.middleTd]}>
        <PositionChips positions={row.mainPosition ? [row.mainPosition] : []} />
      </Box>
      ...
      <Box component='td' sx={[sharedSx.td, sharedSx.middleTd]}>
        <PerformanceTopItems items={row.performanceTopItems} />
      </Box>
...
export default function PerformancePrintContent({ model }) {
  return (
    <Box sx={sx.tableWrap}>
      <Box component='table' sx={sharedSx.table}>
        <TableColumns columns={model.columns} />
        <TableHead columns={model.columns} />
