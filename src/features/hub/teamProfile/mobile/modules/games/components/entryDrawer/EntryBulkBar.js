// teamProfile/mobile/modules/games/components/entryDrawer/EntryBulkBar.js

import { Box, Chip, Divider, Typography, IconButton } from '@mui/joy'

import { iconUi } from '../../../../../../../../ui/core/icons/iconUi.js'
import { entryEditDrawerSx as sx } from './sx/entryEditDrawer.sx.js'

import {
  getGoalsTotal,
  getAssistsTotal,
} from '../../../../../sharedLogic'

function FilterChip({ active, label, onClick, color = 'primary' }) {
  return (
    <Chip
      size="sm"
      variant={active ? 'solid' : 'outlined'}
      color={active ? color : 'neutral'}
      onClick={onClick}
      sx={{ cursor: 'pointer' }}
    >
      {label}
    </Chip>
  )
}

export default function EntryBulkBar({
  isPlayed,
  draft,
  filters,
  onSetFilter,
  onResetFilters,
  onBulkSetOnSquad,
  onBulkResetStats,
}) {
  const rows = draft?.rows || []

  const onSquadTotal = rows.filter((r) => r?.onSquad === true).length
  const onStartTotal = rows.filter((r) => r?.onStart === true).length

  const goalsTotal = getGoalsTotal(rows)
  const assistsTotal = getAssistsTotal(rows)

  const teamGoals = draft?.raw?.goalsFor || 0
  const hasActiveFilters = filters?.squad !== 'all' || filters?.start !== 'all'

  return (
    <Box sx={sx.bulkBar}>
      <Box sx={sx.bulkBarTop}>
        <Typography level="title-sm" startDecorator={iconUi({ id: 'entry' })}>
          רישום כולל
        </Typography>

        <Box sx={sx.bulkBarActions}>
          <Chip size="md" variant="solid" startDecorator={iconUi({ id: 'isSquad' })}>
            נבחרו לסגל:
            <Typography level="title-sm" sx={{ display: 'inline', ml: 1, color: '#fff' }}>
              {onSquadTotal}
            </Typography>
          </Chip>

          <Chip
            size="md"
            variant="solid"
            color={onStartTotal > 11 ? 'danger' : 'neutral'}
            startDecorator={iconUi({ id: 'isStart' })}
          >
            נבחרו להרכב:
            <Typography level="title-sm" sx={{ display: 'inline', ml: 1, color: '#fff' }}>
              {onStartTotal}
            </Typography>
          </Chip>

          <Chip
            size="sm"
            variant="solid"
            color={goalsTotal > teamGoals ? 'danger' : 'neutral'}
            startDecorator={iconUi({ id: 'goals' })}
          >
            שערים:
            <Typography level="title-sm" sx={{ display: 'inline', ml: 1, color: '#fff' }}>
              {goalsTotal} / {teamGoals}
            </Typography>
          </Chip>

          <Chip
            size="sm"
            variant="solid"
            color={assistsTotal > teamGoals ? 'danger' : 'neutral'}
            startDecorator={iconUi({ id: 'assists' })}
          >
            בישולים:
            <Typography level="title-sm" sx={{ display: 'inline', ml: 1, color: '#fff' }}>
              {assistsTotal} / {teamGoals}
            </Typography>
          </Chip>

          <IconButton
            size="sm"
            variant="outlined"
            color="neutral"
            onClick={() => onBulkSetOnSquad(false)}
          >
            {iconUi({ id: 'reset' })}
          </IconButton>

          <IconButton
            size="sm"
            variant="outlined"
            color="neutral"
            onClick={onBulkResetStats}
          >
            {iconUi({ id: 'clear' })}
          </IconButton>
        </Box>
      </Box>

      <Box sx={{ display: 'flex', flexDirection: 'row', gap: 1, flexWrap: 'wrap' }}>
        <Typography level="body-xs" color="neutral">
          סגל: סמן אם השחקן היה בסגל. הרכב: זמין רק אם השחקן היה בסגל.
        </Typography>

        {!isPlayed && (
          <Typography level="body-xs" color="warning">
            המשחק עדיין לא שוחק ולכן שערים, בישולים וזמן משחק נעולים.
          </Typography>
        )}

        <Divider orientation="vertical" />

        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flexWrap: 'wrap', flex: 1 }}>
          <FilterChip
            label="סומנו בסגל"
            color="success"
            active={filters?.squad === 'in'}
            onClick={() => onSetFilter('squad', 'in')}
          />

          <FilterChip
            label="לא סומנו בסגל"
            color="success"
            active={filters?.squad === 'out'}
            onClick={() => onSetFilter('squad', 'out')}
          />

          <FilterChip
            label="סומנו בהרכב"
            active={filters?.start === 'in'}
            onClick={() => onSetFilter('start', 'in')}
            color="success"
          />

          <FilterChip
            label="לא סומנו בהרכב"
            active={filters?.start === 'out'}
            onClick={() => onSetFilter('start', 'out')}
            color="warning"
          />

          <Box sx={{ flex: 1 }} />

          {hasActiveFilters ? (
            <Chip
              size="sm"
              variant="outlined"
              color="danger"
              startDecorator={iconUi({ id: 'reset' })}
              onClick={onResetFilters}
              sx={{ cursor: 'pointer', mr: 'auto' }}
            >
              איפוס
            </Chip>
          ) : null}
        </Box>
      </Box>
    </Box>
  )
}
