// src/features/hub/domain/videos/videoDomain.shared.js
...
export function resolveVideosDomain(entity, filters = {}, deps = {}) {
  const profileEntity = entity || null
  const team = profileEntity?.team || profileEntity
  const videosAll = asArr(profileEntity?.videos)
  ...
  const qn = norm(f.q)
  const keySet = pickKeyIds(profileEntity)
