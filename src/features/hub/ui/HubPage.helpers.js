...
export const buildTabsMeta = (MODE, allowedModes = []) => {
  const allowed = new Set(allowedModes)

  return buildAllTabs(MODE).filter((tab) => allowed.has(tab.value))
}
...
export const buildCreateHandlers = ({ openCreate, context, s }) => ({
  onCreateClub: () =>
    openCreate('club', {}, {
      ...context,
      onDone: (res) => s.selectClubById(res?.id || res?.clubId || null),
    }),

  onCreateTeam: () =>
    openCreate('team', {}, {
      ...context,
      onDone: (res) => s.selectTeamById(res?.id || res?.teamId || null),
    }),
  ...
  onAddTask: (taskContext = {}) =>
    openCreate('task', buildTaskPresetDraft(taskContext), {
      ...context,
      ...taskContext,
    }),
})
