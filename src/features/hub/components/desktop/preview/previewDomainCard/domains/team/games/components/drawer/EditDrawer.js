...
export default function EditDrawer({
    ...
    lifecycle.openLifecycle(
      {
        entityType: 'game',
        id: game.id,
        name: `${game?.rivel || 'משחק'} ${game?.gameDate || ''}`,
      },
      {
  ...
  const status = !isValid
    ? { text: 'יש שדות חובה חסרים או לא תקינים', color: 'warning' }
    : isDirty
    ...
    <DrawerShell
      ...
      header={
        <DrawerHeaderShell
          ...
          subline={`${
            draft?.rivel || game?.rivel || game?.rival || 'משחק'
          } - ${getFullDateIl(draft?.gameDate || game?.gameDate || '')}`}
          titleIconId="games"
          avatar={avatar}
        />
      }
