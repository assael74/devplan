// previewDomainCard/domains/player/games/components/drawerEx/EditExDrawerHeader.js
...
export default function EditExDrawerHeader({ player, game }) {
  const fullName = [player?.playerFirstName, player?.playerLastName].filter(Boolean).join(' ')
  const rival = game?.rivel || game?.rival || game?.game?.rivel || '—'
  const dateLabel = game?.gameDate || game?.dateLabel || game?.dateH || '—'
