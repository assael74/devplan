// src/features/hub/components/preview/previewDomainCard/domains/team/videos/TeamVideosDomainModal.js
import React, { useMemo, useState } from 'react'
import { Box, Typography } from '@mui/joy'
import { getMonthKey, getMonthLabel } from './teamVideos.domain.logic.js'
...
import { enrichVideoAnalysisForTeam } from '../../../../../../../../shared/videoAnalysis/videoAnalysis.enrich'
...
export default function TeamVideosDomainModal({
  ...
  const items = useMemo(
    () => (Array.isArray(team?.videos) ? team.videos : []),
    [team?.videos]
  )
  ...
  const onEdit = (video) => handoff(actions.onEdit, { video, entity: team, context })
  const onLink = (video) => handoff(actions.onLink, { video, entity: team, context })
  ...
  return (
    <Box sx={{ width: '100%', minHeight: 0, display: 'flex', flexDirection: 'column' }}>
      ...
      <Box sx={{ flex: 1, minHeight: 0, overflow: 'auto', px: 1.5, pt: 1 }}>
        {!sorted.length ? (
          <Typography level="body-sm" sx={{ opacity: 0.75 }}>
            אין קטעי וידאו לקבוצה
          </Typography>
        ) : (
          <StickySectionsByMonth
            ...
            renderItem={(video) => {
              const videoEnriched = enrichVideoAnalysisForTeam(video, team, context)
              return (
                <VideoAnalysisCard
                  key={video?.id}
                  video={videoEnriched}
