// src/features/reports/teamTargets/integration/TeamTargetsReportButton.js
...
function closePublishWindow(nextWindow) {
  try {
    ...
    console.warn('[TeamTargetsReportButton] Failed to close placeholder tab', error)
  }
}

function logPublishDebugResult({ input, result }) {
  console.group('[TeamTargetsReportButton] Publish debug result')
...
export default function TeamTargetsReportButton({
  team,
  draft,
  disabled = false,
  iconOnly = false,
  ...
  const handlePublishReport = async () => {
    ...
    try {
      const response = await publishTeamTargetsReport({
        team,
        draft,
      })
      ...
      if (!targetUrl) {
        throw new Error('[TeamTargetsReportButton] Missing currentUrl from publish result')
      }
      ...
      console.error('[TeamTargetsReportButton] Failed to publish report', error)
