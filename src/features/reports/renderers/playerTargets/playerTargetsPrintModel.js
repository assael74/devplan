...
} from '../../model/players/info/print/shared/playerTargetSections.js'
...
const resolveTeamDisplayName = (player, team) => {
  ...
  if (
    !year ||
    String(name).includes(String(year))
  ) {
    return name
  }
...
const resolveCoachName = team => {
  const rules = Array.isArray(team?.rules)
    ? team.rules
    : []
...
function withDisplayState({ model = {}, presentation = 'pdf', isMobile = false } = {}) {
  return {
    ...model,
    presentation,
    isMobile,
...
export function buildPlayerTargetsPrintModel({
  player = {},
  team = {},
  reportDate,
  presentation = 'pdf',
  ...
  const targets = buildPlayerTargetsState({
    player,
    team: activeTeam,
  })
  ...
  const sections = buildPlayerTargetSections({
    profile,
    targets,
  })
  ...
  return withDisplayState({
    ...
    model: {
      ...
      teamDisplayName: resolveTeamDisplayName(
        player,
        activeTeam
      ),
    ...
    return withDisplayState({
      ...
      model: {
        ...
        team:
          ...
          {},
        reportDate:
          inputModel.reportDate ||
          reportDate ||
          new Date(),
        hasTargets:
          inputModel.hasTargets !== false,
      },
    })
  }
