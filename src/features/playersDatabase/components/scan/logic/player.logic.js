// src/features/playersDatabase/components/scan/logic/player.logic.js
...
export const SCAN_LAYER_OPTIONS = [
...
export const normalizeLayer = layer => MIDFIELD_LAYERS.includes(clean(layer)) ? 'midfield' : clean(layer)

export const getLayerOption = layer => SCAN_LAYER_OPTIONS.find(option => option.id === normalizeLayer(layer))

export const getPositionOptions = layer => {
  const option = getLayerOption(layer)
  return option ? option.layers.flatMap(item => POSITION_LAYERS[item] || []) : []
}

export const getPlayerPositionInfo = player => {
  const primaryPosition = clean(player?.primaryPosition)
  const positions = Array.isArray(player?.positions) ? player.positions.filter(Boolean) : clean(player?.positions) ? [clean(player.positions)] : []
  ...
  const inferred = getPlayerGeneralPosition({ primaryPosition, positions })
  return {
    primaryPosition,
    positions,
...
export const getPlayerUrls = player => ({
  playerUrl: clean(player?.playerUrl || player?.source?.playerUrl),
  leagueUrl: clean(player?.leagueUrl || player?.source?.leagueUrl),
  teamUrl: clean(player?.teamUrl || player?.source?.teamUrl),
})
