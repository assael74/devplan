// features/playersDatabase/components/summary/hooks/useSummaryBoard.js
...
const cleanText = value => String(fallback(value)).trim()

const createDetailsForm = (league = {}) => ({
  leagueName: league.leagueName || '',
  ageGroupLabel: league.ageGroupLabel || '',
  level: fallback(league.level),
...
const compareText = (a, b) => cleanText(a).localeCompare(cleanText(b), 'he')
...
const filterLeagues = ({
  ...
  const levelOk =
    levelFilter === 'all' ||
    String(fallback(league?.level)) === levelFilter

  return birthYearOk && levelOk
})

const createSummarySearchParams = ({
  leagueId,
  birthYearFilter,
  levelFilter,
} = {}) => {
    ...
    .map(([key, team]) => {
      ...
      return {
        ...
        currentTeam: {
          ...
          leagueLevel: hasValue(team?.leagueLevel)
            ? team.leagueLevel
            : hasValue(team?.level)
              ? team.level
              : league?.level,
...
export function useSummaryBoard() {
  ...
  const load = useCallback(async (options = {}) => {
    ...
    try {
      const cached = force ? null : getSummaryCache()
  ...
  const openLeagueById = leagueId => {
    ...
    navigate(
      `/players-database/leagues/${encodeURIComponent(targetLeague.id)}`,
      {
        state: {
          league: targetLeague,
          boardFilters: {
