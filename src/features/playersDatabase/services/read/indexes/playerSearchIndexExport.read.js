...
export const readSearchIndexExportById = ({ documentId = '' } = {}) => (
  readSearchIndexDocument({
    documentId,
    action: 'search-index-audit-referral-read',
  })
)
...
export const readTeamSearchIndexesExport = async ({ birthTeamId = '' } = {}) => {
  ...
  // Older and newer team-index writers did not always persist the same
  // identity field. The global audit accepts all three, so the team repair
  // dialog must read the same set or it can incorrectly report a clean team.
  ...
  const snapshots = await Promise.all(identityFields.map(identityField => (
    trackedGetDocs(
      query(
        collection(db, PLAYERS_DATABASE_COLLECTIONS.searchIndexes),
        where(identityField, '==', safeBirthTeamId),
      ),
      {
        feature: 'playersDatabase',
        action: 'team-search-indexes-json-read',
        collection: PLAYERS_DATABASE_COLLECTIONS.searchIndexes,
        meta: { birthTeamId: safeBirthTeamId, identityField },
      }
    )
  )))
  ...
  snapshots.forEach(snapshot => {
    snapshot.docs.forEach(item => {
      ...
      if (clean(row.entityType) === 'birthTeamSeason') {
