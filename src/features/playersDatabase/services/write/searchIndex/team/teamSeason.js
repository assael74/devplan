...
export async function updateTeamSeasonPlayerUrl({
  ...
  const playerUrl = clean(player.playerUrl)
  ...
  if (!teamId) throw new Error('Missing birth team id')
  if (!seasonId) throw new Error('Missing season id')
  if (!playerMatchValues.size) throw new Error('Missing player identity')
  ...
  return runTransaction(db, async transaction => {
    ...
    const players = Array.isArray(seasonRow.teamPlayers)
      ? seasonRow.teamPlayers
      : []
    const playerIndex = players.findIndex(row => (
      buildPlayerMatchValues(row).some(value => (
        playerMatchValues.has(clean(value).toLowerCase())
      ))
    ))
    ...
    const nextPlayers = players.map((row, index) => (
      index === playerIndex
        ? {
            ...row,
            playerUrl,
