...
export const buildPlayerSeasonIndexSyncPlan = ({
  ...
  safePlayers.forEach(player => {
    ...
    operations.push({
      ...
      expected: {
        entityType: 'playerSeason', birthTeamId: teamId, seasonKey,
        exists: Boolean(existingDoc),
        fingerprint: existingDoc ? buildRosterSourceFingerprint(existingData) : '',
      },
    })
  })
  ...
  if (replaceScope) {
    existingDocs.forEach(existingDoc => {
      ...
      operations.push({
        ...
        expected: {
          entityType: 'playerSeason', birthTeamId: teamId, seasonKey, exists: true,
          fingerprint: buildRosterSourceFingerprint(rowData(existingDoc)),
        },
      })
    })
  }
