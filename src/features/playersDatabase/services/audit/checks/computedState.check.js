// src/features/playersDatabase/services/audit/checks/computedState.check.js
...
const buildAuditDataFromSnapshot = ({
  snapshot,
  includeRepairData = false,
}) => {
  const teamDocs = Array.isArray(snapshot?.documents?.teams)
    ? snapshot.documents.teams
    : []
  const playerDocs = Array.isArray(snapshot?.documents?.players)
    ? snapshot.documents.players
    : []
  const playerSearchDocs = Array.isArray(snapshot?.documents?.playerIndexes)
    ? snapshot.documents.playerIndexes
    : []
  const teamSearchDocs = Array.isArray(snapshot?.documents?.teamIndexes)
    ? snapshot.documents.teamIndexes
    : []
  const teamIndexRows = flattenTeamIndexDocs(teamSearchDocs)
  const playerRows = flattenPlayerDocs(playerDocs)
  ...
  const readsUsed = Number(snapshot?.readsUsed || 0)
  ...
  return {
    ...
    cost: {
      audit: buildPlayerScoutAuditCost({
        ...
        playerDocuments: playerDocs.length,
      ...
      readSafety: {
        ...
        remainingBudget: Math.max(
          0,
          PLAYER_SCOUT_AUDIT_READ_SAFETY_LIMIT - readsUsed
        ),
      },
    },
  }
...
export async function buildScopedComputedStateCheck({
  teamDocumentId,
  seasonKey,
  includeRepairData = false,
  readSafetyLimit = PLAYER_SCOUT_AUDIT_READ_BUDGETS.TEAM_SEASON,
...
export async function buildComputedStateCheck({
  includeRepairData = false,
  snapshot = null,
} = {}) {
  const generatedAt = new Date().toISOString()
  const rows = snapshot
    ? buildAuditDataFromSnapshot({
        snapshot,
        includeRepairData,
      })
    : await readAuditData({
...
export const downloadComputedStateCheck = audit => {
  if (!audit) return
