// src/features/playersDatabase/services/audit/repair/directSearchIndex.repair.js
...
const buildMergedFieldsOrThrow = issues => {
  const mergedFields = {}
  ...
  ;(Array.isArray(issues) ? issues : []).forEach(issue => {
    ...
    Object.entries(fields).forEach(([field, value]) => {
      if (
        Object.prototype.hasOwnProperty.call(mergedFields, field) &&
        !sameValue(mergedFields[field], value)
      ) {
        throw new Error(
          'נמצאו ערכים סותרים לאותו שדה בתיקון ישיר של אינדקס.'
        )
      }
...
export const repairSearchIndexIssuesDirect = async ({
  ...
  for (const group of groupedByDocument.values()) {
    ...
    const currentData = snapshot.data() || {}
    const mergedFields = buildMergedFieldsOrThrow(groupIssues)
