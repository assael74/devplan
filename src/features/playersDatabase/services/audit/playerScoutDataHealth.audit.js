...
import { LEAGUES_DATABASE_GENERIC_OBJECTS_CATALOG } from '../../catalog/firestoreDocuments/leagueDocument.catalog.js'
...
const isTypeCompatible = ({ actualValue, expectedValue, path }) => {
  ...
  if (isPlainObject(expectedValue)) return isPlainObject(actualValue)
...
const readScopeDocuments = async ({ scope, config }) => {
  const source = collection(db, config.collectionName)
  const documents = []
  let cursor = null
  let readsUsed = 0

  while (readsUsed < READ_SAFETY_LIMIT) {
    const remaining = READ_SAFETY_LIMIT - readsUsed
    const pageLimit = Math.min(PAGE_LIMIT, remaining)
    const constraints = []

    if (config.entityType) {
      constraints.push(where('entityType', '==', config.entityType))
    }
    ...
    const snapshot = await trackedGetDocsFromServer(
      query(source, ...constraints),
      {
        ...
        operation: 'audit',
        source: `dataHealth:${scope}`,
      }
    )

    readsUsed += snapshot.docs.length
    documents.push(...snapshot.docs)

    if (snapshot.docs.length < pageLimit) break
    if (readsUsed >= READ_SAFETY_LIMIT) {
      throw new Error(
...
export async function buildPlayerScoutDataHealthAudit({ scope } = {}) {
  ...
  const { documents, readsUsed } = await readScopeDocuments({ scope, config })
