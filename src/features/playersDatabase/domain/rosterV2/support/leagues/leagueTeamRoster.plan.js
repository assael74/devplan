...
export const applyApprovedLeagueTeamRosterPatch = ({
  league = {},
  target = {},
  patch = {},
} = {}) => {
  ...
  if (!leagueId || !seasonKey || !birthTeamDocumentId || !sourceTarget) {
    throw new Error('Missing approved League roster target')
  }

  const current = league.current || null
  const history = Array.isArray(league.history) ? league.history : []
  const requested = { seasonKey }
  const currentMatches = sourceTarget === 'current' && isSameSeason(current, requested)
  const historyIndex = sourceTarget === 'history'
    ? history.findIndex(row => isSameSeason(row, requested))
    : -1
  ...
  if (!seasonRow) {
    throw new Error('Approved League season target was not found')
  }
  ...
  const rowIndex = tableRank.findIndex(row => {
    ...
    return rowTeamId === birthTeamDocumentId
  })

  if (rowIndex < 0) {
    throw new Error('Approved League team target was not found')
  }
  ...
  const nextSeason = {
    ...
    playersCount: sumPlayers(nextTableRank),
  }

  return sourceTarget === 'current'
    ? { ...league, current: nextSeason }
    : {
      ...league,
