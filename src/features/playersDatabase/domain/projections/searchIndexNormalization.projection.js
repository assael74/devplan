// Canonical SearchIndex projection metrics. Pure domain calculation; no persistence.
...
const resolveSeasonProjectionState = ({
  ...
  // Remaining games determine whether a projection has work to do; they do
  // not determine the season lifecycle.  Ending a league is an explicit user
  // decision, represented by `seasonStatus` / current-vs-history.
  const canProject = (
    lifecycle.usesProjection &&
    hasProjectionBase &&
  ...
  return {
    ...
    normalizationStatus: lifecycle.seasonStatus === 'not_started'
      ? 'not_started'
