...
function PlayerPageContent() {
  ...
  const [taskCreateSaving, setTaskCreateSaving] = React.useState(false)
  ...
  const handleTaskCreate = async draft => {
    if (taskCreateSaving) return

    const row = historyView.selectedRow || {}
    setTaskCreateSaving(true)
  ...
  return (
    <>
      <Box sx={sx.page}>
        ...
        <Box sx={sx.contentGrid}>
          ...
          <PlayerActionsPanel
            ...
            onTaskCreate={() => setTaskCreateOpen(true)}
      ...
      <PlayerTaskCreateModal
        open={taskCreateOpen}
        busy={taskCreateSaving}
        onClose={() => !taskCreateSaving && setTaskCreateOpen(false)}
