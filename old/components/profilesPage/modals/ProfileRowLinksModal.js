// features/playersDatabase/components/profilesPage/modals/ProfileRowLinksModal.js
...
import { modalSx as sx } from './modal.sx.js'

export default function ProfileRowLinksModal({
  open,
  row,
  saving = false,
  error = '',
      ...
      <ModalDialog sx={sx.dialog}>
        ...
        <Box sx={sx.head}>
          ...
          <Box>
            ...
            <Typography level="body-sm" sx={sx.meta}>
              קישורים לשחקן ולקבוצה, עם אפשרות להרחיב בהמשך להערות ושדות נוספים.
            </Typography>
          </Box>
        </Box>

        <Box sx={sx.identity}>
          <Typography level="body-sm" sx={sx.identityTitle}>
            {playerName || '-'}
          </Typography>
        ...
        <Box sx={sx.actions}>
          <Button
            ...
            color="primary"
            loading={saving}
            onClick={() => onSave?.({ playerUrl, teamUrl })}
          >
            שמור שורה
          </Button>
