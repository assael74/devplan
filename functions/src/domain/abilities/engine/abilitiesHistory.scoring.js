// functions\src\domain\abilities\engine\abilitiesHistory.scoring.js

// קובץ מקביל: src/shared/abilities/engine/abilitiesHistory.scoring.js
...
const {
  ...
  DOMAIN_RELIABILITY,
  OVERALL_RELIABILITY,
} = require('./abilitiesHistory.constants')
const {
  isFilled,
  round1,
  round2,
...
const { shouldUseGrowthStage } = require('../abilitiesAgeRule')
const { normalizeAbilities } = require('./abilitiesHistory.forms')
const {
  groupFormsByWindow,
  buildWindowSnapshot,
  mergeWindowAbilitiesHistory,
} = require('./abilitiesHistory.windows')
...
function calcPotentialScore(domainsById = {}, growthStage = null, playerContext = {}) {
...
function resolveOverallReliability({
  validDomainsCount = 0,
  coveragePct = 0,
  evaluatorsCount = 0,
}) {
...
function buildFinalPlayerResult({ forms = [], player = null } = {}) {
...
module.exports = {
  getDomainsMeta,
  calcDomainFromAbilities,
  calcDomainsResult,
  calcWeightedOverallFromDomains,
  calcPotentialScore,
  resolveOverallReliability,
  resolvePotentialReliability,
